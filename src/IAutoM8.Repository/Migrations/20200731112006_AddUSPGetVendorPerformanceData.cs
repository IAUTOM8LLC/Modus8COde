...
{
    public partial class AddUSPGetVendorPerformanceData : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var query = @"CREATE PROCEDURE [dbo].[uspGetVendorPerformanceData]
...
FROM   (SELECT DISTINCT anu.id, 
        ...
        FROM   aspnetusers anu 
               ...
               INNER JOIN userprofile up 
                       ON up.userid = anu.id 
               INNER JOIN formulataskvendor FTV 
                       ON FTV.vendorguid = anu.id 
               INNER JOIN formulatask FT 
                       ON FTV.formulataskid = FT.id 
               INNER JOIN formulaproject FP 
                       ON FT.formulaprojectid = FP.id  and ( (fp.isglobal=1 and fp.status = 5) or (fp.IsGlobal = 0) ) -- newly added by nik to just show publishyed admin formula or custom formulas
               LEFT JOIN skill S 
                  ...
                  WHERE  [value] IS NOT NULL 
                         ...
                         AND EXISTS (SELECT * 
                                     ...
                                     WHERE  ( FTS_2.vendorguid = FTS.vendorguid 
                                            ...
                                            AND FTS_2.completed IS NOT NULL) 
                  GROUP  BY vendorguid, 
                            formulataskid) RATING 
              ON VENDORS.id = RATING.vendorguid 
                 AND VENDORS.formulataskid = RATING.formulataskid 
       LEFT JOIN (SELECT vendorguid, 
                         formulataskid, 
                  ...
                  GROUP  BY vendorguid, 
                            formulataskid) DWELL 
              ON VENDORS.id = DWELL.vendorguid 
                 AND VENDORS.formulataskid = DWELL.formulataskid 
                                 ...
                                 AND EXISTS (SELECT * 
                                             ...
                                             WHERE  ( FTS_2.vendorguid = 
                                                    ...
                                                    AND FTS_2.completed IS NOT 
                                                        NULL)) A 
                  GROUP  BY formulataskid) MEAN_DWELL 
              ON VENDORS.formulataskid = MEAN_DWELL.formulataskid 
       LEFT JOIN (SELECT vendorguid, 
                         formulataskid, 
                         Cast(Avg(Cast (Replace(value, '-', '') AS 
                                        DECIMAL(15, 2))) AS 
                  ...
                  GROUP  BY vendorguid, 
                            formulataskid) CT 
              ON VENDORS.id = CT.vendorguid 
                 AND VENDORS.formulataskid = CT.formulataskid 
       LEFT JOIN (SELECT formulataskid, 
                         Cast(Avg(Cast (vlu AS DECIMAL(15, 2))) AS 
                              DECIMAL(15, 2)) AS 
                                   MEANCT 
                          ...
                          WHERE  [value] IS NOT NULL 
                                 ...
                                 AND EXISTS (SELECT * 
                                             ...
                                             WHERE  ( FTS_2.vendorguid = 
                                                    ...
                                                    AND FTS_2.completed IS NOT 
                                                        NULL))A 
                  GROUP  BY formulataskid) MEAN_CT 
              ON VENDORS.formulataskid = MEAN_CT.formulataskid 
ORDER  BY vendor_id, 
          skillname, 
          formulaid
END";
        ...
        {
            migrationBuilder.Sql(@"DROP PROCEDURE IF EXISTS [dbo].[uspGetVendorPerformanceData];");
        }
    }
}
