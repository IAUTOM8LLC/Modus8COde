...
{
    public partial class ModifyStoreProcPublishFormula : Migration
    {
...
ALTER PROCEDURE [dbo].[usp_PublishFormula]           
 @FormulaID INT          
 AS          
BEGIN         
BEGIN TRY    
...
delete ftd
 ...
 and (ftadminparent.id is null or ftadminchild.id is null)
...
UPDATE FTClient          
 ...
 ,FTClient.IsInterval=FTAdmin.IsInterval          
 ,FTClient.IsShareResources=FTAdmin.IsShareResources          
 ,FTClient.AssignedSkillId=SClient.Id          
 ,FTClient.ReviewingSkillId=SReviewClient.Id          
 ,FTClient.TeamId=TClient.ID          
 ,FTClient.DescNotificationFlag=FTAdmin.DescNotificationFlag    
 ,FTClient.ReviewerTraining=FTAdmin.ReviewerTraining       
 ...
 --FROM FormulaTask FTADMIN    
 --INNER JOIN FormulaTask FTCLIENT ON FTADMIN.Id = FTCLIENT.PublicVaultFormulaTaskID    
 --INNER JOIN TEAM TADMIN ON FTADMIN.TeamId = TADMIN.Id    
 --INNER JOIN TEAM TCLIENT ON TCLIENT.PublicVaultTeamID = TADMIN.Id --AND TCLIENT.OwnerGuid = FTCLIENT.OwnerGuid    
 --INNER JOIN SKILL SADMIN ON FTADMIN.AssignedSkillId  = SADMIN.Id    
 --INNER JOIN SKILL SCLIENT ON SCLIENT.PublicVaultSkillID = SADMIN.Id --AND FTCLIENT.OwnerGuid = SCLIENT.OwnerGuid    
 --LEFT JOIN Skill SREVIEWADMIN ON FTADMIN.ReviewingSkillId = SREVIEWADMIN.Id    
 --LEFT JOIN SKILL SREVIEWCLIENT ON SREVIEWCLIENT.PublicVaultSkillID = SREVIEWADMIN.Id --AND FTCLIENT.OwnerGuid = SREVIEWCLIENT.OwnerGuid    
 --WHERE FTAdmin.FormulaProjectId=@FormulaID      
     
 --FROM FormulaTask FTClient          
 --INNER JOIN FormulaTask FTAdmin          
 --ON FTClient.PublicVaultFormulaTaskID=FTAdmin.ID          
