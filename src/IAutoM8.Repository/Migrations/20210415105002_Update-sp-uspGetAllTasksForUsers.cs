...
{
    public partial class UpdatespuspGetAllTasksForUsers : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var sql = @"/****** Object:  StoredProcedure [dbo].[uspGetAllTasksForUsers]    Script Date: 15-04-2021 16:18:11 ******/
SET ANSI_NULLS ON
GO
...
DROP PROCEDURE IF EXISTS [dbo].[uspGetAllTasksForUsers] 
GO 


CREATE PROCEDURE [dbo].[uspGetAllTasksForUsers]
...
, CASE WHEN A.Status = 'OVERDUE' THEN 1
	...
	WHEN A.STATUS = 'TODO-NOTSTARTED' THEN 4
	WHEN A.STATUS = 'REVIEW' THEN 5
	WHEN A.STATUS = 'REVIEW-NOTSTARTED' THEN 6
	WHEN A.STATUS = 'COMPLETED' THEN 7
	WHEN A.STATUS = 'UPCOMING' THEN 8
...
--USERS UNDER OWNER ASSGINED TASKS
	...
	--IN-HOUSE USERS UNDER OWNER or self owner ASSIGNED TASKS FOR TO-DO-NOTSTARTED TASKS WITH NO PROCESSING USERGUID YET (To-Do Task not started yet by anyone)
	--'TODO-NOTSTARTED' --WILL SHOW START ICON AGAINST THESE TASKS
	SELECT DISTINCT 
	NULL AS ID, NULL AS FullName, NULL AS ROLE
	--anu.Id,up.FullName, anr.Name AS ROLE
	,CASE WHEN anu.OwnerId IS NOT NULL THEN anu.OwnerId WHEN anu.OwnerId IS NULL THEN ANU.ID END AS OwnerId --NEW CASE ADDED TO HANDLE SELF OWNER OR TEAM UNDER OWNER
	...
	, pt.Title AS PROJECTTASKTITLE
	, CASE
		 WHEN pt.[Status] = 1 THEN 'TODO-NOTSTARTED'
       END AS [Status]
	, fp.[Name] AS FormulaName
	, pt.FormulaTaskId
	, pt.StartDate
	...
	INNER JOIN UserProfile up on up.UserId=anu.Id
	INNER JOIN ProjectTaskUser PTU ON ANU.Id = PTU.UserId AND PTU.ProjectTaskUserType = 0
	INNER JOIN ProjectTask PT ON PTU.PROJECTTASKID = PT.ID AND pt.Status in  (1) AND PT.ProccessingUserGuid IS NULL--AND PTU.UserId = PT.ProccessingUserGuid
	INNER JOIN Project p ON pt.ProjectId = p.Id
	LEFT JOIN FormulaTask ft ON ft.Id = pt.FormulaTaskId
	LEFT JOIN FormulaProject fp ON fp.Id = ft.FormulaProjectId
	WHERE P.DateCreated >= DATEADD(DAY,-7,GETUTCDATE())
	AND PTU.USERID  IN (@INPUT_PRIMARYUSERID) --NEWLY ADDED

		UNION 

	--IN-HOUSE USERS UNDER OWNER or self owner ASSIGNED TASKS FOR TO-DO TASKS WITH NO PROCESSING USERGUID YET (To-Do Task not started yet by anyone)
	SELECT DISTINCT 
	NULL AS ID, NULL AS FullName, NULL AS ROLE
	--anu.Id,up.FullName, anr.Name AS ROLE
	,CASE WHEN anu.OwnerId IS NOT NULL THEN anu.OwnerId WHEN anu.OwnerId IS NULL THEN ANU.ID END AS OwnerId --NEW CASE ADDED TO HANDLE SELF OWNER OR TEAM UNDER OWNER
	...
	LEFT JOIN FormulaProject fp ON fp.Id = ft.FormulaProjectId
	WHERE P.DateCreated >= DATEADD(DAY,-7,GETUTCDATE())
	AND PTU.USERID NOT IN (@INPUT_PRIMARYUSERID) --NEWLY ADDED TO NOT SHOW START ICON AGAINST THESE
	and pt.Id  NOT IN 
		(
		SELECT DISTINCT pt.Id FROM 
		AspNetUsers anu
		INNER JOIN AspNetUserRoles anur on anur.UserId=anu.Id
		INNER JOIN AspNetRoles anr on anr.Id=anur.RoleId and anr.Name<>'Vendor'
		INNER JOIN UserProfile up on up.UserId=anu.Id
	...
	--VENDOR ASSIGNED UNDER OWNER FOR TO-DO TASKS WITH NO PROCESSING USERGUID YET (To-Do Task not started yet by anyone)
	SELECT DISTINCT 
	NULL AS ID, NULL AS FullName, NULL AS ROLE
	--anu.Id,up.FullName, anr.Name AS ROLE
	--,CASE WHEN anu.OwnerId IS NOT NULL THEN anu.OwnerId WHEN anu.OwnerId IS NULL THEN ANU.ID END AS OwnerId --NEW CASE ADDED TO HANDLE SELF OWNER OR TEAM UNDER OWNER
	...
	--INNER JOIN ProjectTaskUser PTU ON ANU.Id = PTU.UserId AND PTU.ProjectTaskUserType = 0
	INNER JOIN ProjectTaskVendor PTV ON ANU.Id = PTV.VendorGuid
	INNER JOIN ProjectTask PT ON PTV.PROJECTTASKID = PT.ID AND pt.Status in  (1) AND PT.ProccessingUserGuid IS NULL--AND PTU.UserId = PT.ProccessingUserGuid
	INNER JOIN Project p ON pt.ProjectId = p.Id
	LEFT JOIN FormulaTask ft ON ft.Id = pt.FormulaTaskId
	LEFT JOIN FormulaProject fp ON fp.Id = ft.FormulaProjectId
	WHERE P.DateCreated >= DATEADD(DAY,-7,GETUTCDATE())
	...
	UNION 
	--'REVIEW-NOTSTARTED' TO BE REVIEWED TASKS not yet started (pt.status = 2 and pt.reviewinguserguid is null)
	--'REVIEW-NOTSTARTED' - TO SHOW START BUTTON AGAINST THESE
	SELECT DISTINCT 
		--pt.OwnerGuid AS ID, --set to ownerguid here for displaying in owner's dashboard
		NULL AS ID,
		 NULL AS FullName, --set to null so as to show this as 'not assigned'
		  NULL AS ROLE 
	--anu.Id,up.FullName, anr.Name AS ROLE
	,CASE WHEN anu.OwnerId IS NOT NULL THEN anu.OwnerId WHEN anu.OwnerId IS NULL THEN ANU.ID END AS OwnerId --NEW CASE ADDED TO HANDLE SELF OWNER OR TEAM UNDER OWNER
	--anu.Id,up.FullName, anr.Name AS ROLE,anu.Id AS OwnerId
	 ,p.[Name] AS PROJECTNAME
	, P.Id AS PROJECTID
	, pt.Id AS PROJECTTASKID
	, pt.Title AS PROJECTTASKTITLE
	, CASE
         ...
         --WHEN (pt.[Status] = 1 and  ((CAST( DATEDIFF(MINUTE, CAST(pt.StartDate AS DATETIME), GETDATE()) AS INT) >= ( CAST( ROUND(( pt.Duration * 0.8 ), 0) AS INT) ) ) AND ( CAST(DATEDIFF(MINUTE, CAST(pt.StartDate AS DATETIME), GETDATE() ) AS INT) <= CAST(pt.Duration AS int)) )) THEN 'ATRISK' 
         --WHEN pt.[Status] = 1 THEN 'TODO'
		 WHEN pt.[Status] = 2 THEN 'REVIEW-NOTSTARTED'
		 --WHEN pt.[Status] = 3 THEN 'COMPLETED'
       END AS [Status]
	, fp.[Name] AS FormulaName
	, pt.FormulaTaskId
	...
	INNER JOIN UserProfile up on up.UserId=anu.Id
	INNER JOIN ProjectTaskUser PTU ON ANU.Id = PTU.UserId AND PTU.ProjectTaskUserType = 1
	INNER JOIN ProjectTask PT ON PTU.PROJECTTASKID = PT.ID AND pt.Status = 2 AND PT.ReviewingUserGuid is null --and  PTU.UserId = PT.ReviewingUserGuid
	INNER JOIN Project p ON pt.ProjectId = p.Id
	LEFT JOIN FormulaTask ft ON ft.Id = pt.FormulaTaskId
	LEFT JOIN FormulaProject fp ON fp.Id = ft.FormulaProjectId
	WHERE P.DateCreated >= DATEADD(DAY,-7,GETUTCDATE())
	...
	UNION --TO BE REVIEWED TASKS not yet started (pt.status = 2 and pt.reviewinguserguid is null)
	...
	LEFT JOIN FormulaProject fp ON fp.Id = ft.FormulaProjectId
	WHERE P.DateCreated >= DATEADD(DAY,-7,GETUTCDATE())
	AND PTU.USERID  NOT IN (@INPUT_PRIMARYUSERID) --NEWLY ADDED
	AND PT.ID NOT IN  
		(
		SELECT DISTINCT PT.ID FROM 
		AspNetUsers anu
...
(
	...
	--IN-HOUSE USERS UNDER OWNER or self owner ASSIGNED TASKS FOR TODO-NOTSTARTED TASKS WITH NO PROCESSING USERGUID YET (To-Do Task not started yet by anyone)
	--'TODO-NOTSTARTED' --WILL SHOW START ICON AGAINST THESE TASKS
	SELECT DISTINCT 
	NULL AS ID, NULL AS FullName, NULL AS ROLE
	--anu.Id,up.FullName, anr.Name AS ROLE
	...
	UNION 
	--TO BE REVIEWED TASKS not yet started (pt.status = 2 and pt.reviewinguserguid is null)
	--NO START BUTTON REQ
	SELECT DISTINCT 
		--pt.OwnerGuid AS ID, --set to ownerguid here for displaying in owner's dashboard
		NULL AS ID,
...
(	--assigned users data
	...
	--IN-HOUSE USERS ASSIGNED TASKS FOR TODO-NOTSTARTED TASKS WITH NO PROCESSING USERGUID YET (To-Do Task not started yet by anyone)
	--'TODO-NOTSTARTED' --WILL SHOW START ICON AGAINST THESE TASKS
	SELECT DISTINCT anu.Id,up.FullName, anr.Name AS ROLE,anu.OwnerId
	 ,p.[Name] AS PROJECTNAME
	, P.Id AS PROJECTID
	...
	--IN-HOUSE USERS ASSIGNED TASKS FOR TO-DO TASKS WITH NO PROCESSING USERGUID YET (To-Do Task not started yet by anyone)
	--NO START BUTTON TO BE SHOWED FOR THESE
	SELECT DISTINCT anu.Id,up.FullName, anr.Name AS ROLE,anu.OwnerId
	 ,p.[Name] AS PROJECTNAME
	, P.Id AS PROJECTID
	, pt.Id AS PROJECTTASKID
